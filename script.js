...
for (let i = 0; i < 4; i++) {
    ...
    <div class="package-en-container">
        <input type="checkbox" class="package-active-check" id="${idGenerator('enable', idNum)}" unchecked>
        <label for="${idGenerator('enable', idNum)}" class="package-enable">enable</label>
    </div>
    <div class="package-name">${idGenerator('Package', idNum)}</div>
    <div class="package-blocker"></div>
    <div class="image-holder">
        <input type="file" accept="image/*" name="image" id="${idGenerator('file', idNum)}" style="display:none;"
...
let featuresAddButton = Array.from(document.querySelectorAll('.feature-add-button'));
let featureContainers = Array.from(document.querySelectorAll('.features-container'));

let featureDefaultBoxes = Array.from(document.querySelectorAll('.feature-box-default'));
...
for (let index in featuresAddButton) {
    featuresAddButton[index].addEventListener('click', () => { addFeatures(index) })
}
...
function createChildElement(parent, childType, attributeType, attValue, content) {
...
let packActBtns = Array.from(document.querySelectorAll('.package-active-check'));
